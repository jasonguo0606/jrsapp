...
class PlaybackRepository(
    ...
    suspend fun resolveVideoSources(pageUrl: String): Result<List<VideoSource>> =
        withContext(Dispatchers.IO) {
            runCatching {
                ...
                resolveRecursively(pageUrl, depth = 0).distinctBy { it.url }
            }.onFailure {
                Log.e(TAG, "解析播放源失败: $pageUrl", it)
            }
        }

    private fun resolveRecursively(pageUrl: String, depth: Int): List<VideoSource> {
        if (depth > MAX_DEPTH) return emptyList()

        val html = fetchHtml(pageUrl)
        Log.d(TAG, "depth=$depth fetched pageUrl=$pageUrl htmlLen=${html.length} head=${html.take(500)}")
        ...
        nestedPages.forEach { nestedUrl ->
            ...
            val sources = resolveRecursively(nestedUrl, depth + 1)
    ...
    private fun fetchHtml(url: String): String {
        ...
        Log.d(TAG, "fetchHtml request url=$url referer=$url")
        return client.newCall(request).execute().use { response ->
            response.body?.string().orEmpty()
                .also {
                    Log.d(
                        TAG,
                        "fetchHtml response url=$url code=${response.code} finalUrl=${response.request.url} bodyLen=${it.length}"
                    )
                }
        }
    }
